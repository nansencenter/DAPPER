    ...
    def wrapped(self):
        ...
        if _top_dog is None:
            ...
            if _linewidth is None:
                try:
                    _linewidth,h = shutil.get_terminal_size()
                except:
                    _linewidth = 79
...
class AlignedDict(dict):
    ...
    @_init
    def __str__ (self): return self._repr(is_repr=False)
    @_init
    def __repr__(self): return self._repr(is_repr=True)
...
if __name__ == "__main__":
    ...
    # Other tests
    print("\ndeep_getattr:\n================")
    key =  "self.self.self.self.a"
    print(key, ":", deep_getattr(dd,key))
