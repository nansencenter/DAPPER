        ...
        **kwargs
        ...
        for name, (show_by_default, init) in potential_LPs.items():
            if (
                (figlist == ["all"])
                or (name in figlist)
                or (figlist == ["default"] and show_by_default)
            ):
...
class sliding_diagnostics:
    ...
    def __init__(self, fignum, stats, key0, plot_u, E, P, Tplot=None, **kwargs):
        ...
        styles["Values"] = {
            ...
            "iters": [lin(0, 0.1), "dirac", dict(c="m", label="iters/10")],
        ...
        fig, axs = place.freshfig(
            fignum, figsize=(5, 1 + nAx), nrows=nAx, sharex=True, gridspec_kw=GS
        )
...
class weight_histogram:
    ...
    def __init__(self, fignum, stats, key0, plot_u, E, P, **kwargs):
        if not hasattr(stats, "w"):
            ...
            return
        fig, ax = place.freshfig(fignum, figsize=(7, 3), gridspec_kw={"bottom": 0.15})
...
class correlations:
    ...
    def __init__(self, fignum, stats, key0, plot_u, E, P, **kwargs):
        ...
        if Nx <= 1003:
            ...
            trfm = mpl.colors.SymLogNorm(
                linthresh=0.2, linscale=0.2, base=np.e, vmin=-1, vmax=1
            )
    ...
    def init(fignum, stats, key0, plot_u, E, P, **kwargs):
        ...
        def update(key, E, P):
            ...
            for Ens in EE:  # If E is duplicated, so must the others be.
                ...
                if True:
                    d.y.insert(ind, yy[ko, iiY] if ko is not None else nan * ones(Ny))
                if True:
...
def phase_particles(
    ...
    def init(fignum, stats, key0, plot_u, E, P, **kwargs):
        ...
        if "E" in d:
            h.E += [
                ax.plot(*xn, **p.ens_props)[0] for xn in np.transpose(d.E, [1, 2, 0])
            ]
        if "mu" in d:
...
def spatial1d(
    ...
    def init(fignum, stats, key0, plot_u, E, P, **kwargs):
        ...
        if p.conf_mult:
            lines_s = ax.plot(
                ii, nan1, "b-", lw=1, label=(str(p.conf_mult) + r"$\sigma$ conf")
            )
    ...
    def init(fignum, stats, key0, plot_u, E, P, **kwargs):
        ...
        for ax in [ax_13, ax_23]:
            ax.yaxis.set_tick_params(
                "major", length=2, width=0.5, direction="in", left=True, right=True
            )
